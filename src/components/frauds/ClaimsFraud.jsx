...
const ClaimsFraud = props => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 15; i++) {
      newFrauds.push({
        transactionNo: '12345678790',
        duplicateFound: 'Yes',
        idProof: '2424 4244 4244',
        fraudLevel: 2,
        date: '14/02/2019'
      })
    }
  ...
  return <Styled>
    ...
    <table className="table">
      ...
      <tr>
        ...
        <th scope="col">Duplicate Found</th>
...
const ClaimsFraud = () => {
  ...
  useEffect(() => {
    if (updateRequired) {
      axios({
        method: 'get',
        url: `${process.env.REACT_APP_API_URL}/frauds/claim/all`
      })
        .then(res => {
          setFrauds(res.data);
  ...
  const updateFraud = async () => {
    const data = JSON.stringify({
      "idProofNo": selectedCard.idProofNo,
      "duplicate": selectedCard.duplicate,
    });
    // check if adding new card or updating existing card
    if (selectedCard.user) {
      axios({
        method: 'put',
        url: `${process.env.REACT_APP_API_URL}/frauds/claim/update?id=${selectedCard.id}`,
        headers: {
      ...
      const config = {
        method: 'post',
        url: `${process.env.REACT_APP_API_URL}/frauds/claim/add?user_id=${user.userId}`,
        headers: {
          'Content-Type': 'application/json'
        },
        data: data
  ...
  const deleteCard = async () => {
    var config = {
      method: 'post',
      url: `${process.env.REACT_APP_API_URL}/frauds/claim/delete`,
      headers: {
        'Content-Type': 'application/json'
      },
      data: {
  ...
  return <Styled>
    ...
    <table className="table">
      ...
      {frauds.map((fraud, index) => <tr onMouseEnter={() => setSelectedCard(fraud)}>
        <th scope="row">{index + 1}</th>
        <td>{fraud.idProofNo}</td>
        <td>{fraud.duplicate ? 'YES' : 'No'}</td>
        <td>
      ...
      <div className="modal-dialog">
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title" id="exampleModalLabel">Claims Details</h5>
          ...
          <div className="modal-body">
            <table className='table'>
              <tr>
                <td>Id Proof No</td>
                <td>: {selectedCard?.idProofNo}</td>
              </tr>
              <tr>
                <td>Duplicate</td>
                <td>: {selectedCard?.duplicate ? 'Yes' : 'No'}</td>
              </tr>
              <tr>
                <td>Block Status</td>
      ...
      <div className="modal-dialog">
        <div className="modal-content">
          ...
          <div className="modal-footer">
            ...
            <button type="button" className="btn btn-primary" data-bs-dismiss="modal" onClick={updateFraud}>Save
            </button>
          </div>
        </div>
      </div>
